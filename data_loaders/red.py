...
class RED(Dataset):
    ...
    def preprocess(self):
        for cls in self.cls_list:
            ...
            setattr(self, "train_" + cls + "_landmarks", [])
            ...
            for i, line in enumerate(lines):
                ...
                getattr(self, "train_" + cls + "_landmarks").append(splits[2])

        for cls in self.cls_list:
            setattr(self, "test_" + cls + "_filenames", [])
            setattr(self, "test_" + cls + "_mask_filenames", [])
            setattr(self, "test_" + cls + "_landmarks", [])
            ...
            for i, line in enumerate(lines):
                ...
                getattr(self, "test_" + cls + "_landmarks").append(splits[2])
    ...
    def __getitem__(self, index):
        if self.mode == 'train' or self.mode == 'train_finetune':
            ...
            landmarks_A = np.load(os.path.join(self.image_path, getattr(self, "train_" + self.cls_A + "_landmarks")[index_A]))
            landmarks_B = np.load(os.path.join(self.image_path, getattr(self, "train_" + self.cls_B + "_landmarks")[index_B]))
            return self.transform(image_A), self.transform(image_B), self.transform_mask(mask_A), self.transform_mask(mask_B), landmarks_A, landmarks_B
