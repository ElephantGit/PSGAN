...
class Solver_PSGAN(object):
    ...
    def train(self):
        ...
        for self.e in range(start, self.num_epochs):
            for self.i, (img_A, img_B, mask_A, mask_B, landmarks_A, landmarks_B) in enumerate(self.data_loader_train):
                ...
                for i in range(64):
                    for j in range(64):
                        landmarks_org[0, :, i, j] = np.append([i]*68 - landmarks_A[0][:,0].numpy(), [j]*68 - landmarks_A[0][:, 1].numpy())
                        landmarks_ref[0, :, i, j] = np.append([i]*68 - landmarks_B[0][:,0].numpy(), [j]*68 - landmarks_B[0][:, 1].numpy())
                landmarks_org = self.to_var(F.interpolate(torch.Tensor(landmarks_org), scale_factor=1/4), requires_grad=False)
                landmarks_ref = self.to_var(F.interpolate(torch.Tensor(landmarks_ref), scale_factor=1/4), requires_grad=False)

                # ================== Train D ================== #
                # training D_X, D_X aims to distinguish src image
                # Real
                out = getattr(self, "D_" + cls_A)(org_A)
                d_loss_real = self.criterionGAN(out, target_is_real=True)
                # Fake
                fake_A = self.G(org_A, ref_B, landmarks_org, landmarks_ref, mask_org, mask_ref)
                fake_B = self.G(ref_B, org_A, landmarks_ref, landmarks_org, mask_ref, mask_org)
                fake_A_detach = Variable(fake_A.data).detach()
                fake_B_detach = Variable(fake_B.data).detach()
                out = getattr(self, "D_" + cls_A)(fake_A_detach)
                d_loss_fake =  self.criterionGAN(out, target_is_real=False)
                ...
                # training D_Y, D_Y aims to distinguish class A
                # Real
                out = getattr(self, "D_" + cls_B)(ref_B)
                d_loss_real = self.criterionGAN(out, target_is_real=True)
                # Fake
                out = getattr(self, "D_" + cls_B)(fake_B_detach)
                d_loss_fake =  self.criterionGAN(out, target_is_real=False)
                ...
                if (self.i + 1) % self.ndis == 0:
                    ...
                    if self.lambda_idt > 0:
                        ...
                        idt_A = self.G(org_A, ref_B, landmarks_org, landmarks_org, mask_org, mask_ref)
                        idt_B = self.G(ref_B, org_A, landmarks_ref, landmarks_ref, mask_ref, mask_org)
                    ...
                    g_A_loss_adv = self.criterionGAN(pred_fake, target_is_real=True)
                    # GAN loss D_B(G_B(B))
                    pred_fake = getattr(self, "D_" + cls_B)(fake_B)
                    g_B_loss_adv = self.criterionGAN(pred_fake, target_is_real=True)
                    rec_B = self.G(fake_B, fake_A, landmarks_ref, landmarks_org, mask_ref, mask_org)
                    rec_A = self.G(fake_A, fake_B, landmarks_org, landmarks_ref, mask_org, mask_ref)
